...
class DependencyDownloader:
    ...
    def _get_latest_frida_version(self):
        ...
        try:
            ...
            print(f"Latest Frida version: {self.actual_version}")
            return self.actual_version
        except requests.RequestException as e:
            print(f"Error fetching latest Frida version: {e}")
            return None
    ...
    def _construct_frida_server_url(self, version_to_use):
        ...
        print(f"Constructed Frida server URL: {url}")
        return url
    ...
    def _extract_xz(self, xz_path, output_path):
        ...
        print(f"Extracting {xz_path} to {output_path}...")
        try:
            ...
            print(f"Error extracting .xz file {xz_path}: {e}")
            return False
        except IOError as e:
            print(f"Error writing extracted file to {output_path}: {e}")
            return False
    ...
    def check_and_download_frida_server(self):
        ...
        if self.version.lower() == "latest":
            ...
            if not latest_ver:
                print("Could not determine latest Frida version. Cannot proceed.")
                return None
            version_to_use = latest_ver
        else:
            self.actual_version = self.version # Use the specified version
            print(f"Using specified Frida version: {self.actual_version}")
        ...
        if os.path.exists(final_binary_path):
            print(f"Frida server binary already exists: {final_binary_path}")
            return final_binary_path

        print(f"Frida server binary not found. Attempting download for version {self.actual_version}...")
        ...
        try:
            os.remove(temp_xz_path)
            print(f"Cleaned up {temp_xz_path}.")
        except OSError as e:
            print(f"Warning: Could not remove temporary file {temp_xz_path}: {e}")

        print(f"Frida server successfully downloaded and extracted to: {final_binary_path}")
        return final_binary_path 